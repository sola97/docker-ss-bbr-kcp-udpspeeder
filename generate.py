...
#基本参数
CLIENT_FASTOPEN= "--fast-open" #只对ssclient有效,禁用时候置为空即可
FEC="1:3,2:4,8:6,20:10" #UDPspeeder的fec参数
PASSWD="passwd" #密码，记得替换
ENCRYPT="aes-256-cfb" #加密方式，并不是所有都兼容，我用chacha20时发现udp不通
...
def ss_kcptun_udpspeeder(server_num=0, client_offset=0,suffix=""):
    ...
    -p {server_udpspeeder_port+server_num*2}:6501/udp \\\n\
     sola97/shadowsocks \\\n\
    -s "ss-server" \\\n\
    -S "-s 0.0.0.0 -p 6443 -m {ENCRYPT} -k {PASSWD} -u --fast-open\" \\\n\
    -k "kcpserver" \\\n\
    -K "-l 0.0.0.0:6500  -t 127.0.0.1:6443 -mode fast2" \\\n\
    -u "-s -l0.0.0.0:6501 -r 127.0.0.1:6443  -f{FEC} -k {PASSWD}"'
    ...
    sola97/shadowsocks \\\n\
    -s "ss-local" \\\n\
    -S "-s 127.0.0.1 -p 6500 -b 0.0.0.0 -l 1080 -u -m {ENCRYPT} -k {PASSWD}  {CLIENT_FASTOPEN}" \\\n\
    -k "kcpclient"  \\\n\
    -K "-l :6500 -r {server_ip}:{server_kcptun_port+server_num*2} -mode fast2" \\\n\
    -u "-c -l[::]:6500  -r{server_ip}:{server_udpspeeder_port+server_num*2} -f{FEC} -k {PASSWD}"'
...
def ss_kcptun_udpspeeder_udp2raw(server_num=0, client_offset=0,suffix=""):
    ...
    -p {client_socks5_port+client_offset}:1080/udp \\\n\
    sola97/shadowsocks \\\n\
    -t "-c -l0.0.0.0:3333  -r{server_ip}:{server_udp2raw_port+server_num*2}  -k {PASSWD} --cipher-mode xor --auth-mode simple --raw-mode faketcp -a\" \\\n\
    -T "-c -l0.0.0.0:3334  -r{server_ip}:{server_udp2raw_port+1++server_num*2}  -k {PASSWD} --cipher-mode xor --auth-mode simple --raw-mode faketcp -a\" \\\n\
    -k "kcpclient"  \\\n\
    -K "-l :6500 -r 127.0.0.1:3333 -mode fast2" \\\n\
    -u "-c -l[::]:6500  -r127.0.0.1:3334 -f{FEC} -k {PASSWD}" \\\n\
    -s "ss-local" \\\n\
    -S "-s 127.0.0.1 -p 6500 -b 0.0.0.0 -l 1080 -u -m {ENCRYPT} -k {PASSWD}  {CLIENT_FASTOPEN}"'
...
if __name__ == '__main__':
    ...
    cl_offset=input(f"客户端将使用{client_socks5_port+client_offset}端口，如需修改请输入新的偏移量，当前为{server_num}\n：")
    ...
    while True:
        print("请选择方案：")
        print("[0].退出")
        print("[1].SS + Kcptun + UDPspeeder")
        print("[2].SS + Kcptun+UDP2raw + UDPspeeder+UDP2raw")
        print("[3].同时运行[1]和[2]")
        ...
        if input_select == "3":
            ss_kcptun_udpspeeder(server_num=server_num, client_offset=client_offset,suffix="_udp")
            ss_kcptun_udpspeeder_udp2raw(server_num=server_num+1, client_offset=client_offset+1,suffix="_tcp")
            break
        if input_select =="0":
            print("退出")
            break
